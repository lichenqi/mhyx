...
import com.lianliantao.yuetuan.adapter.TiXianRecordAdapter;
...
public class TiXianRecordFragment extends LazyBaseFragment {
    Context context;
    int pageNum = 1;
    ...
    ImageView nodata;
    List<TixianRecordBean.WithdrawInfoBean> list = new ArrayList<>();
    ...
    private void iniview() {
        ...
        adapter = new TiXianRecordAdapter(list, context);
    ...
    private void getData() {
        ...
        MyApplication.getInstance().getMyOkHttp().post().tag(this)
                .url(CommonApi.BASEURL + CommonApi.TIXIAN_RECORD_LIST + mapParam)
                .enqueue(new JsonResponseHandler() {
                    @Override
                    public void onSuccess(int statusCode, JSONObject response) {
                        super.onSuccess(statusCode, response);
                        Log.i("提现记录", response.toString());
                        TixianRecordBean bean = GsonUtil.GsonToBean(response.toString(), TixianRecordBean.class);
                        if (bean.getErrno() == CommonApi.RESULTCODEOK) {
                            List<TixianRecordBean.WithdrawInfoBean> withdrawInfo = bean.getWithdrawInfo();
                            if (pageNum == 1) {
                                if (withdrawInfo.size() == 0) {
